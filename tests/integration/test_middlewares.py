...
from tests.test_start_handler import make_update
...
async def wired_dispatcher(session_factory):
    ...
    return build_dispatcher(
        settings, storage=MemoryStorage(), session_factory=session_factory
    )
...
async def test_start_records_the_user(
    ...
    await wired_dispatcher.feed_update(bot, make_update('/start'))
...
async def test_group_updates_are_dropped_before_the_database(
    ...
    await wired_dispatcher.feed_update(bot, make_update('/start', 'group'))
