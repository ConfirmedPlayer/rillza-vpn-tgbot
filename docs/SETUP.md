...
## Канал алертов: необязателен только рядом с человеком
...
**На сервере без присмотра эта пара обязательна**, и вот почему. Отказ
панели — неверный `PANEL_API_KEY`, отозванный ключ, недоступная сеть —
не роняет джобу: `PanelError` ловится внутри `PaymentService._provision`
и `ReconcileService.run`, пишется как `WARNING`, и джоба завершается
**успешно**. Значит heartbeat зелёный, healthcheck зелёный, `docker ps`
зелёный — а люди платят и не получают доступ. Единственный автоматический
сигнал в этом случае — та самая строка `WARNING`, и уйти ей некуда, кроме
этого канала.

Проверять надо не наличие переменных, а доставку: сломайте
`PANEL_API_KEY`, дождитесь сообщения в чате, верните ключ обратно.
Ненастроенный канал выглядит точно так же, как канал, в котором нет
проблем.
