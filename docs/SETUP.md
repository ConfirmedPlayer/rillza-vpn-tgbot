...
3. Ещё раз `check_panel`. Число аккаунтов вырастет, а список
   `overriding` — нет: новый аккаунт создан с `maxDevices=0` и попал
   под лимит группы.

Если новый аккаунт окажется в списке `overriding` или без группы —
значит `POST /api/users` ушёл без `groups`, и его VLESS не уедет на
ноду.
