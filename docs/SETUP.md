...
Шаг 2 сам по себе не доказывает, что `groups` доехал до `POST
/api/users` и Xray-пуш прошёл: кнопка HAPP успешно подключится и от
одной только Hysteria (пуш ей не нужен вообще, см. PLAN.md §8.2), а
панель отвечает `201` даже когда пуш в Xray упал молча — рабочего
VLESS вживую пока никто не наблюдал. Проверка, которая это
действительно доказывает: `GET /api/users/{tg_id}` — поле `groups` не
должно быть пустым.
...
## Живость: чем проверяется и чего проверка не даёт

`restart: unless-stopped` реагирует только на процесс, который
**завершился**. Корутина, зависшая без падения, оставляет контейнер
поднятым: бот не отвечает никому, а снаружи всё выглядит здоровым.
Без присмотра — на VPS — это может тянуться сутками.

Поэтому у сервиса `bot` есть healthcheck: `scripts/healthcheck.py`
спрашивает у базы, когда любая фоновая задача в последний раз
отработала. Две самых быстрых тикают раз в 30 и 60 секунд, так что пять
минут полной тишины (`HEALTHCHECK_QUIET` в `app/core/jobs.py`) — это не
затянувшийся проход, а остановившийся планировщик. Вопрос задаётся
снаружи зависшего процесса, поэтому зависание не может на него
соврать.

```bash
docker compose ps          # видно healthy / unhealthy
docker inspect --format '{{.State.Health.Status}}' rillza-vpn-tgbot-bot-1
```

**Чего это не даёт.** Docker сам по себе **не перезапускает**
нездоровый контейнер — это умеет только Swarm. Проверка делает
состояние видимым, а действовать по нему должно что-то на хосте:
внешний сторож вроде `willfarrell/autoheal`, или строчка в cron,
перезапускающая контейнеры со статусом `unhealthy`. Пока такого сторожа
нет, healthcheck — это сигнал для человека, а не самолечение.

`start_period` в 180 секунд закрывает первый запуск на пустой базе,
когда ни одна задача ещё не отработала.
