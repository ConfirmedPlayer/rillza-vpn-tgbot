...
- **Тарифы по сроку и числу устройств** (хранятся в БД, цены и
  активность редактируются из админки без деплоя; сид-миграции кладут
  эти значения): один и тот же набор сроков продаётся дважды, на 2 и
  на 4 устройства. Цены четырёхустройственного набора — цены
  двухустройственного, умноженные на 1.6, поэтому процент выгоды за
  длинный период одинаковый в обеих сетках. Заголовок тарифа называет
  число устройств явно (`title_ru` вида «1 месяц · до 2 устройств»),
  чтобы счёт и экран провайдера не оставляли гадать, за какой тариф
  идёт оплата.

  | Код | Срок | Устройства | Цена | В месяц | Выгода |
  |---|---|---|---|---|---|
  | `m1` | 1 месяц | 2 | 200 ₽ | 200 ₽ | — |
  | `m3` | 3 месяца | 2 | 540 ₽ | 180 ₽ | −10 % |
  | `m6` | 6 месяцев | 2 | 960 ₽ | 160 ₽ | −20 % |
  | `m12` | 12 месяцев | 2 | 1680 ₽ | 140 ₽ | −30 % |
  | `m1x4` | 1 месяц | 4 | 320 ₽ | 320 ₽ | — |
  | `m3x4` | 3 месяца | 4 | 864 ₽ | 288 ₽ | −10 % |
  | `m6x4` | 6 месяцев | 4 | 1536 ₽ | 256 ₽ | −20 % |
  | `m12x4` | 12 месяцев | 4 | 2688 ₽ | 224 ₽ | −30 % |

  В интерфейсе показываем цену за месяц и процент выгоды — так сетка
  продаёт длинные периоды сама. Число устройств идёт в панель как
  `maxDevices`, явно, а не как «наследовать группу» (см. §7).
