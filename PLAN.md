...
серверной группе Rillza, Provider ID не покупаем. Бот создаёт пользователей с
`maxDevices: 0` — это «наследовать лимит группы», то есть 2. Так лимит
меняется в одном месте (группа), а не переписыванием всех пользователей.
`permissive` выбран осознанно: он ограничивает только клиенты, которые шлют
HWID (Happ), и не ломает подписку тем, кто открыл её другим приложением.
...
бот учитывает асимметрию провижининга (§8).

Осталось решить перед запуском: включать ли Soft-block для истёкших подписок
(рекомендую да — человек увидит понятный текст вместо ошибки).
...
Settings → Subscription → Happ → HWID mode = `permissive`; опционально
Soft-block для истёкших.
